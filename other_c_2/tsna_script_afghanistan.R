...
input <- "/Users/christian.conroy/OneDrive - Accenture Federal Services/Documents/StateGec/Brandwatch/Afghanistan/Data"
setwd(input) #set working directory

# Amount of lines to skip 
skip_lines = 9

## finds all files ending in .xlsx in working directory
...
# Set Datasource 
data_source_crimson = FALSE
data_source_feather = FALSE
data_source_feather_raw = TRUE
data_source_brandwatch = FALSE
...
gec_pal <- c("Not Credible" = "#C00000", "Neutral" = "#E7E6E6", "Credible" = "#439c40")  
...
# 1. For Interactive Custom Visualizations Custom Graph (If TRUE), you need to adjuat the ifelse to reflect your categories and colors for directed and undirected
...
#################################### Load in Data and Convert to Edgelist for CH if needed ##########################
get_dynamic_edgelist_feather <- function(x){
  # This function generates an edgelist for the data. Authors are individuals who wrote tweets.
  # targets are individuals within the tweets. T
  print('importing feather data...')
  raw <- x %>%
...
x <- files
##################
get_dynamic_edgelist_feather_raw <- function(x){
  # This function generates an edgelist for the data. Authors are individuals who wrote tweets.
  # targets are individuals within the tweets. T
  raw <- list.files(path = x,
                    pattern = "*.feather",
                    full.names = T)
 
  #raw <- raw %>% 

  print('importing feather data raw...')
  raw <- raw %>%
    map_df(~read_feather(.)) %>%
    mutate(authors = paste0("@", screen_name))
...
get_dynamic_network <- function(x, data_source){
  # Cut down the dataset to a manageable size based on the interaction_level set
  # Change back to normal report when done
  if(data_source_crimson == TRUE & data_source_feather == FALSE & data_source_brandwatch == FALSE & data_source_feather_raw == FALSE){
  time_edgelist_both <- reduce_network(get_dynamic_edgelist_crimson(x))
  }else if(data_source_crimson == FALSE & data_source_feather == TRUE & data_source_brandwatch == FALSE & data_source_feather_raw == FALSE){
    time_edgelist_both <- reduce_network(get_dynamic_edgelist_feather(x))
  }else if(data_source_crimson == FALSE & data_source_feather == FALSE & data_source_brandwatch == FALSE & data_source_feather_raw == TRUE){
    time_edgelist_both <- reduce_network(get_dynamic_edgelist_feather_raw(x))
  }else if(data_source_crimson == FALSE & data_source_feather == TRUE & data_source_brandwatch == TRUE & data_source_feather_raw == FALSE){
    time_edgelist_bw <- reduce_network(get_dynamic_edgelist(str_subset(x, "mentions")))
    time_edgelist_fth <- reduce_network(get_dynamic_edgelist_feather(str_subset(x, "mentions", negate = TRUE)))
    time_edgelist_both <- mapply(rbind, time_edgelist_bw, time_edgelist_fth, SIMPLIFY = FALSE)
  } 
  else{
    time_edgelist_both <- reduce_network(get_dynamic_edgelist(x))
  }
...
#undirected <- mentions_network_dynamic
#directed <- mentions_network_dynamic_directed
#i = 22
...
reachable_paths <- function(undirected, directed) {
  ...
  undirected <-networkDynamic(network.list=listofnets_un)
  directed <-networkDynamic(network.list=listofnets_di)
  ...
  if(custom_reachable_output == TRUE){
  ...
  for (i in bf[,"V2"]) {
    ...
    if((length(fwd_reach_dir$tdist[between(fwd_reach_dir$tdist, 0, Inf, incbounds = FALSE) == TRUE]) >0) == TRUE) {
      if(custom_reachable_labels == TRUE){
      ...
      plot(fwd_reach_dir, main=paste0("Chains from accounts that mentioned ", account_name, " over time"), label = directed %v% 'fwd_label', edge.lwd = 1, edge.label.cex = .8, jitter = TRUE, edge.label.col = "black", label.cex = 1, mode = 'fruchtermanreingold')
        ...
        directed %v% 'fwd_label' <- ifelse(directed %v% 'label' %in% unlist(bf[bf$V2 == i,][,2]), directed %v% 'label', NA)
        
        par(mar=c(0, 0, 1, 1))
        plot(fwd_reach_dir, main=paste0("Chains from accounts that mentioned ", account_name, " over time"), label = directed %v% 'label', edge.lwd = 1, edge.label.cex = .8, jitter = TRUE, edge.label.col = "black", label.cex = 1, mode = 'fruchtermanreingold')
    ...
    plot(bkw_reach_dir, main=paste0("Chains from accounts that were mentioned by ", account_name, " over time"), edge.col = rgb(0, 97, 255, max=255, alpha=166), label = directed %v% 'label', edge.lwd = 1, edge.label.cex = 0.8, jitter = TRUE, edge.label.col = "black", label.cex = 1, mode= "fruchtermanreingold")
      } else {
        directed %v% 'bwd_label' <- ifelse(directed %v% 'label' %in% unlist(bf[bf$V2 == i,][,2]), directed %v% 'label', NA)
      ...
      plotPaths(
        ...
        edge.label.cex = 0.7,
        label.cex = 1,
        main = paste0(account_name, " was not mentioned by other accounts")
      )
    }
...
new_zoom_viz <- function(x) {
  ...
  if(i == 1){
    ...
    network_slice <- network.collapse(x, onset = (time_start + ((i_zoom-1)* interval_val)), terminus = (time_start + ((i_zoom)* interval_val)), rule = "any")
    network_slice_title <- paste(anytime((time_start + ((i_zoom-1)* interval_val)), tz = "America/New_York"), anytime((time_start + ((i_zoom)* interval_val)), tz = "America/New_York"), sep = " - ") 
    
  }else {
    network_slice <- network.collapse(x, onset = (time_start + ((i_zoom-1)* interval_val)), terminus = time_end, rule = "any")
    network_slice_title <- paste(anytime((time_start + ((i_zoom-1)* interval_val)), tz = "America/New_York"), round.POSIXt(anytime(time_end, tz = "America/New_York"), unit = "day"), sep = " - ") 
  }
  
  # Nodes to retain 
  bf <- as.data.frame(cbind(network_slice %v% c("label"), network_slice %v% c("id")))
  bf$V2 <- as.numeric(as.character(bf$V2))
  bf <- bf[bf$V1 %in% reply_nodes_zoom,]
  ...
  coords <- as.matrix(coords_df)
    ...
    ggplot(ggnetwork(network_slice, layout = coords), aes(x = x, y = y, xend = xend, yend = yend)) +
        ...
        geom_nodelabel_repel(aes(label = label),
                             box.padding = unit(1, "lines"),
                             size = 6,
                             data = function(x) { x[ x$label %in% label_names_zoom, ]})+
        theme_blank()+
        theme(legend.text=element_text(size=12))  + ggtitle(network_slice_title) + labs(colour="Network Status")
    
    ggsave(paste0(output_folder, 'custom_zoom/', 'time_slice_', i, sprintf('_zoomed_replythread_%s.png', as.character(interaction_level))), height = 9, width = 11, units = "in")
    dev.off()
}
