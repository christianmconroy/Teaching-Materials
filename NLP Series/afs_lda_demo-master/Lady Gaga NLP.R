...
setwd("~/OneDrive - Accenture Federal Services/Documents/Onboarding/Training Opps/NLP Series/afs_lda_demo-master")
...
# grab all the songs # Genius package was used to pull the album data 
music_cache = c()
...
# Manual on contractions... there's prob an easier way to do... 
...
# Start Here!! ----------------------------------------------------------- Above is all that gets you to the csv

gaga_genius3 <- read_csv('gaga.csv')
...
# Most corpuses follow a zip distribution and stopwords are used most, so drop in BoW approaches
# SMART lexicon pretty good here - it pulls out the word help though! So not perfect. So good to understand. 
...
# We know we need to drop purely melodic and need to get rid of some of the noise 

# preprocessing ---------------------------------------------------------------------

df <- read_csv('gaga.csv')
df <- df %>%
...
dtmat <- as.matrix(dtm)
# Creates a document term matrix - each row is a document and each column represents words (count of number of times word appears in a song)
...
#pyLDAvis in Python
# Lambda used for finding words that are most important for a specific topic but not for others 


# AP LDA ----------------------------------------------------------------

# Lets see it perform better... Yeah, so news is better than songs for LDA
...
# Direchlet distributions occur on simplexes, or k-dimensional triangles 
  # At start of algorithm, documents appear randomly throughout the pyramid and then topics are randomly altered until there is a good 
  # reputation 

  # Imagine mummies partying in the middle of a pyramid and they are all wearing rainbow colors, but there are different shades
  # It gets a little late, so the pharoa says go away 

  # The mummies need to run to wherever they live in a random corner of the pyramid, and they want to be with people like them 
  # They take turns running around to the corners assessing each time whether they have commonalities with others 
  # Happens thousands and thousands of time until they end up where they should 

  # LDA is a form of dimensionality reduction

  # LDA effectively wiggles the words and topics and uses the change and probabilities to change and optimize 

# What is the different between entity topic modeling and correlational topic modeling for what GEC is doing...?  

# To evaluate the number of k to use with k, you have to give it a number of k topics 
    # We can use perplexity here (why perplexity vs coherence)?
    # K-folds cross validation for perplexity - but computationally intensive 
    # Easier is to use coherence - what I usually used

# Can pull out hypaxies by filtering by TFIDF

# Lang detect packages are all really slow... 
      # Compare NER script I am working with with Laura's LDA_Mallet script
      # Maybe just straight up steal her pre-processing and then run the NER from there...
      # Just need to ensure that whatever she is doing supports all the languages I care about. 
